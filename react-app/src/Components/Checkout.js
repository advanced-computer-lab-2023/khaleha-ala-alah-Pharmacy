...
const StripePaymentButton = ({ amount, patientId }) => {
 ...
 patientId =  '652b512450d1b797fa0a42ef';
 ...
 const onToken = (token) => {
    ...
    fetch('http://localhost:4000/patients/save-stripe-token', {
      ...
      .then(async (data) => {
        ...
        if (data.success) {
          ...
          const orderResponse = await fetch(`http://localhost:4000/patients/checkout/${patientId}`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
  ...
  try {
    ...
    const walletResponse = await fetch('http://localhost:4000/patients/remove-from-wallet', {
      method: 'POST',
      headers: {
    ...
    if (walletResponse.ok) {
      ...
      console.error('Error decrementing wallet amount:', walletResponse.statusText);
      alert('Error placing order. Please try again.');
    }
  } catch (error) {
    console.error('Error processing payment:', error);
  ...
  const handleCashOnDelivery = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:4000/patients/checkout/${patientId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
