...
const PatientHomePagebuy = () => {
  ...
  useEffect(() => {
    ...
    const newTotalQuantity = Object.values(cart).reduce((total, item) => total + item.quantity, 0);
      ...
      <div className="medicine-list">
        {filteredMedicines.map((medicine) => (
          <div key={medicine._id} className="medicine-card-container">
            ...
            <div>
              <button onClick={() => updateCart(medicine._id, 'subtract')}>-</button>
              <span>{cart[medicine._id]?.quantity || 0}</span>
              <button onClick={() => updateCart(medicine._id, 'add')}>+</button>
              <button onClick={() => updateCart(medicine._id, 'add')}>Add to Cart</button>
            </div>
          </div>
        ))}
      </div>
